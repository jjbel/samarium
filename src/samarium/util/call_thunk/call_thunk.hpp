...
#elif defined(_M_X64) || defined(__x86_64__)
#if defined(_WIN32)
    return _size == sizeof(char) || _size == sizeof(short) || _size == sizeof(int) ||
...
class base_thunk
{
  protected:
    ...
    void init_code(call_declare caller,
                   call_declare callee,
                   size_t argc,
                   const argument_info* arginfos = NULL) /* CHANGED: throw(bad_call) */;
    void destroy_code();
    void flush_cache();
...
{
  public:
    explicit unsafe_thunk(size_t argc,
    ...
    template <typename T, typename PROC> void bind(T& object, PROC proc)
    {
        bind_impl(static_cast<void*>(&object), *(void**)&proc);
        flush_cache();
    }

    template <typename Callback> operator Callback() const
    {
...
template <typename Arg> inline size_t stat_param_bytes(size_t bytes)
{
    if (std::is_pointer<Arg>::value || std::is_reference<Arg>::value) bytes += sizeof(intptr_t);
    else
        bytes += (sizeof(Arg) + sizeof(intptr_t) - 1) / sizeof(intptr_t) * sizeof(intptr_t);
    return bytes;
}

template <typename CallerRet, typename... CallerArgs> class thunk_impl : public base_thunk
{
  public:
  ...
  private:
    ...
    template <typename> struct check_call;
...
#define __CALL_THUNK_CHECK_CALL__(declare, ...)                                      \
    ...
    template <size_t N> inline void get_argument_infos(argument_info (&arginfos)[N])
    {
...
template <typename CallerRet> class thunk_impl<CallerRet> : public base_thunk
{
  public:
    thunk_impl(call_declare caller = default_caller, call_declare callee = default_caller)
    {
  ...
  private:
    ...
    struct check_call_impl
        : public std::integral_constant<bool, std::is_same<CallerRet, CalleeRet>::value>
    {
    };
...
template <typename> struct get_call_declare;
...
template <typename> class thunk;

template <typename CallerRet, typename... CallerArgs>
class thunk<CallerRet(__FASTCALL__*)(CallerArgs...)> : public thunk_impl<CallerRet, CallerArgs...>
{
  public:
    ...
    operator CallbackType() const { return reinterpret_cast<CallbackType>(this->_code); }
};

#if defined(_M_IX86) || defined(__i386__)

template <typename CallerRet, typename... CallerArgs>
class thunk<CallerRet(__CDECL__*)(CallerArgs...)> : public thunk_impl<CallerRet, CallerArgs...>
{
  public:
    typedef thunk_impl<CallerRet, CallerArgs...> base_class;
...
template <typename CallerRet, typename... CallerArgs>
class thunk<CallerRet(__STDCALL__*)(CallerArgs...)> : public thunk_impl<CallerRet, CallerArgs...>
{
  public:
    typedef thunk_impl<CallerRet, CallerArgs...> base_class;
