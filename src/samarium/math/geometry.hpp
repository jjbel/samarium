...
[[nodiscard]] constexpr auto lerp_along(Vector2 point,
                                        const LineSegment& ls) noexcept
{
...
[[nodiscard]] constexpr auto project(Vector2 point,
                                     const LineSegment& ls) noexcept
{
...
[[nodiscard]] constexpr auto project_clamped(Vector2 point,
                                             const LineSegment& ls) noexcept
{
    // https://stackoverflow.com/a/1501725/17100530
    const auto vec = ls.vector();
...
[[nodiscard]] constexpr auto distance(Vector2 point,
                                      const LineSegment& ls) noexcept
{
...
[[nodiscard]] constexpr auto lies_in_segment(Vector2 point,
                                             const LineSegment& l) noexcept
{
...
intersection(const LineSegment& l1, const LineSegment& l2) noexcept
{
...
clamped_intersection(const LineSegment& l1, const LineSegment& l2) noexcept
{
...
[[nodiscard]] constexpr auto abs_area(Circle circle) noexcept
{
    return math::abs(area(circle));
}
} // namespace sm::math
